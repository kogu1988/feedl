...
npm run lint      # ESLint
npx vitest run    # 360 birim testi
...
| Katman | Sonuç | Kapsam |
| :--- | :--- | :--- |
| **Birim test** (`npm test`) | ✅ 53 dosya · **360 test geçti** | Saf mantık: renk/WCAG, sayfalama, CSV, şifreleme, rate-limit, Paddle imza+plan türetme, **hesap düzeyi Pro devralma kararı**, oy doğrulama, post-format, outcome kaydı (gelir ayrıştırma/biçimleme + API şeması), **workspace oluşturma limiti (Free 1 workspace)**, data-export kapsamı/redaksiyonu, post-search, widget-origins, widget gömme (body-bekleme), workspace-host çözümleme, **fail-closed host politikası**, AI içgörüleri, OpenRouter modelleri, e-posta teslimatı, haftalık digest e-postası + gönderim kararı, api-keys, davet e-postası, widget gönderim modu, free-plan oy limiti, **entegrasyon URL token doğrulaması**, **Linear webhook tenant izolasyonu**, **widget self-embed kapıları (host + oturum)**, **`getWorkspaceId` önceliği**, **middleware public allowlist'i** |
...
| **E2E smoke** (`test:e2e`) | ✅ CI'da GERÇEKTEN koşar (`e2e` job'u, `DATABASE_URL` secret'ı tanımlı): **42 passed / 3 skipped** (axe a11y taraması **12** herkese açık rotada: `/`, `/portal`, `/roadmap`, `/changelog`, `/demo`, `/alternative`, `/how-to-*`, `/privacy`, `/terms`, `/refund`, `/contact`). Landing testi host-aware — CI'da `NEXT_PUBLIC_APP_URL=localhost` olduğu için GERÇEK landing doğrulanır. **Host izolasyonu** (`e2e/host-isolation.spec.ts`) gerçek 404 statüsünü, robots/sitemap kapatmasını ve kök-host markının `feedl` kaldığını doğrular | Ana yüzeyler + public API yüzeyi (OpenAPI, v1 401 gate) + axe a11y |
...
| **DB-backed entegrasyon** (Sprint 71.1) | ✅ `TEST_DATABASE_URL` setliyse koşar (yoksa ATLANIR — `npm test` etkilenmez). CI'da `e2e` job'unda, ayrı Neon branch'ine karşı koşar; kendi fikstürünü `afterAll`'da siler. **Neden kritik:** 64 migration/37 tablonun gerçek şemaya karşı doğrulanması — bu sınıf (SQL/şema regresyonu) birim testle yakalanamaz | `tests/integration/revenue-score-tenant.db.test.ts` — gelir skoru SQL'inin **kiracı izolasyonu** (eski #22 sızıntı sınıfı: A'nın MRR'i B'nin skoruna sızamaz) |
| **Yük probu** (Sprint 71.4) | ✅ Salt-okunur; üretime karşı güvenle koşulur (`tools/load-probe.mjs`). Canlı ölçüm (10 eşzamanlı, 3 tur): `/` p50 908ms · `/portal` p50 929ms · `/roadmap` p50 674ms · `/api/health` p50 284ms — **0 hata**. p95'i Hobby cold start yükseltir; regresyon sinyali p50'dedir | — |
| **Migration sıfırdan kurulum** (Sprint 71.2) | ✅ `node tools/verify-migrations.mjs` — geçici bir `pgvector/pgvector:pg16` konteyneri açar, **64 migration'ın tamamını boş bir DB'de** sırayla uygular, konteyneri siler. Sonuç: **64/64 · 37 tablo**. Bulut/DB sırrı gerekmez; CI'da ayrı `migrations` job'u olarak koşar | `tools/verify-migrations.mjs` |
...
- **SPRINT 65–67 (2026-09-13) — ölçüm, dogfood, hijyen.** Ayrıntı: `docs/FEEDL-ROADMAP.md`'nin ölçüm eksiği (glm_analyse.md §3.3) kapatıldı: **birinci taraf `analytics_events`** tablosu (migration `0062`) + tek kaynak taksonomi (`lib/analytics/names.ts`) + `trackEvent` (`after()` ile yanıt sonrası yazar; Vercel'de kayıp olmaz) ve aktivasyon sayfasında **olay tabanlı huni**. Huni adımları gerçek kaynaklarında tetikleniyor (signup/workspace_created/feedback_added/customer_linked/revenue_added/priority_viewed/upgrade_clicked). **Dogfood (Sprint 66):** portal artık boş değil — 14 gerçek fikir + 4 şirket + 3 fırsat + 16 oy yazıldı (`tools/seed-dogfood.mjs`, idempotent, `--clean` ile geri alınabilir); AI hattı **gerçek veriyle** doğrulandı (14/14 özet + duygu + tür + embedding). **Hijyen (Sprint 67):** `docs/` ayrı private repo'da (`feedl-docs`) yedeklendi, vite 8 geri alındı (rolldown native binding Windows'ta yerel vitest'i kırıyordu; .gitignore + dependabot ignore notu eklendi), `SKILL.md`'deki rakip marka referansları temizlendi ve middleware notları güncellendi.
- **SPRINT 69–70 (2026-09-13) — TR-first ve dosya bölünmesi.** **Hedef pazar kararı: TR-first** (`docs/decision-tr-first.md`) — ürün zaten Türkçe ve ilk görüşmeler yerel; i18n kanıt yokken yapılmayacak. TL fiyat gösterimi **env ile açılır** (`getPriceDisplay`, `NEXT_PUBLIC_PRICE_*`): üç TL değeri de yoksa otomatik USD'ye düşer — uydurma kur veya yarı yapılandırılmış karışık para birimi gösterilmez (testli). Ayrıca hesap düzeyi Pro kuralı ve deneme↔cayma hakkı ayrımı yüzeylerde yazılı hale getirildi. **Bölünme (Sprint 70):** `inngest/functions.ts` (1256) → `inngest/functions/{ai-autopilot,notify-*,send-webhooks,corpus-insights,weekly-digest,shared}.ts` + barrel (çağrı yeri değişmedi); `app/(main)/dashboard/page.tsx` (1246) → sayfa **688** + `lib/dashboard/loaders.ts` **498**; `components/custom/companies-manager.tsx` (1100) → `components/custom/companies/{companies-manager,company-form-dialog,add-member-dialog,opportunity-form-dialog,shared}.tsx`. Fonksiyon gövdeleri birebir taşındı; davranış değişmedi.
- **Faz 3:**
  ...
  - **#11** ikinci tenant'la gerçek çok kiracılı canlı kanıt — **ÇÖZÜLDÜ (2026-09-13).** **Host düzeyi (2026-09-12):** prod'da iki tenant var — `feedl` (pro) ve `deneme` (free, aynı owner). `deneme.feedl.app/portal` → 200, `feedl.app/portal` → 200, var olmayan `kesinlikleyok12345.feedl.app/portal` → **404** (fail-closed politika). **İÇERİK düzeyi de artık kanıtlı (2026-09-13, Sprint 66):** `feedl` workspace'ine 14 gerçek fikir + 4 şirket + 3 fırsat yazıldı (`tools/seed-dogfood.mjs`), `deneme`'ye `IZOLASYON-TESTI-…` işaretli 1 fikir eklendi. Sunucu doğrulaması: `select count(*) from posts where workspace_id=<feedl> and title=<deneme marker>` → **0**; workspace başına sayım `feedl=14`, `deneme=1`. HTTP doğrulaması: `feedl.app/portal` yalnız feedl fikirlerini, `deneme.feedl.app/portal` yalnız kendi fikrini render ediyor. Yani tenant izolasyonu hem host hem **içerik** düzeyinde canlı olarak ispatlandı.
