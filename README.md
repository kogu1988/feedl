...
| **DB-backed entegrasyon** (Sprint 71.1) | ✅ `TEST_DATABASE_URL` setliyse koşar (yoksa ATLANIR — `npm test` etkilenmez). CI'da `e2e` job'unda, ayrı Neon branch'ine karşı koşar; kendi fikstürünü `afterAll`'da siler. **Neden kritik:** 62 migration/36 tablonun gerçek şemaya karşı doğrulanması — bu sınıf (SQL/şema regresyonu) birim testle yakalanamaz | `tests/integration/revenue-score-tenant.db.test.ts` — gelir skoru SQL'inin **kiracı izolasyonu** (eski #22 sızıntı sınıfı: A'nın MRR'i B'nin skoruna sızamaz) |
