...
| 20 | **EKSİK — custom domain'in TRAFİK yarısı yok (2026-09-12 doğrulandı).** Sahiplik doğrulaması çalışıyor (biçim/rezerve host reddi, `_feedl.<domain>` TXT token'ı, doğrulanmamış alanın host çözümlemesinde yok sayılması, unique index — migration `0056`) ve uygulama host→workspace eşleşmesini DB'den yapıyor. AMA müşteriye **trafiği bize çevirecek CNAME söylenmiyor** (`lib/custom-domain.ts`, ayarlar arayüzü ve dokümanlarda `cname`/vercel geçmiyor) ve domain **Vercel projesine eklenmiyor** (hesapta tek alakalı domain `feedl.app`; müşteri domaini hiç eklenmemiş). Yani TXT doğrulanmış bir müşteri domaini bugün trafik ALMAZ — happy path teknik olarak imkânsız. Çözüm iki yoldan biri: (a) uygulamaya Vercel API entegrasyonu (`VERCEL_API_TOKEN` + proje id → domaini projeye ekle + CNAME hedefini arayüzde göster; gerçek self-servis), (b) her müşteri domaini için elle `vercel domains add <domain> feedl` + arayüzde CNAME talimatı (bugün yapılabilir, ölçeklenmez). `test.feedl.app` KULLANILAMAZ (rezerve + zaten slug routing). En azından negatif yollar denenebilir; happy path bir karara bağlı. | Operasyon | 3 | 3 | 2 | 27 |
...
- **Faz 1 (bitti):** README/mimari doğruluğu (#9), orta ve düşük borçların kapatılması. `tsc` + `lint` + `vitest` (**245**) + `build` + e2e yeşil.
...
- **Faz 2 — kalan:**
  - **#20** custom domain — yukarıdaki satır: sahiplik var, **trafik yönlendirmesi YOK** (karar bekliyor: Vercel API entegrasyonu mu, elle ekleme mi).
...
- **Karar bekleyenler (senden — kod hazır, yön kararı yok):**
  - `components/ui/toast.tsx` **ölü iskelet** (base-ui toast; 0 kullanım, hiç mount edilmemiş) — sil / bırak / bağla?
  - **`router.refresh()` yeterli mi?** Tam sayfa yenilemeler kaldırıldı (flash/kaydırma yok), ama 22 dosyada hâlâ sunucu bileşenleri yeniden çekiliyor. "Veri hiç çekilmesin, liste yerel state'ten güncellensin" isteniyorsa bu bir RSC veri katmanı yeniden yazımıdır.
  - **Free workspace limiti daha sıkı olsun mu?** Şu anki kural: sahip olduğun workspace'lerden biri etkin Pro ise ek workspace açabilirsin (ödeyen müşteri kısıtlanmaz).
  - **Resend `mail.feedl.app` = `partially_failed`:** gönderim TAM sağlıklı (DKIM+SPF doğrulanmış, son e-postalar `delivered`, 0 suppression, `EMAIL_FROM` = `feedl <no-reply@mail.feedl.app>`). Durum yalnızca **kullanılmayan gelen (MX) kaydından** geliyor — kodda inbound e-posta yok. Ya gelen özelliğini kapat (rozet temizlenir, tek çağrı, geri alınabilir) ya MX kaydını ekle (ileride yanıtla-ve-gönder için gerekir). Ayrıca `TrackingCAA` kaydı `pending`.
  - **`DATABASE_URL_UNPOOLED` Vercel'de `Config`** (değeri panelde/CLI'da düz okunabilir) — aynı kimlik bilgileri taşıyor; Sensitive'a çevrilmeli mi? (Neon entegrasyonu oluşturdu, körlemesine değiştirmedim.)
